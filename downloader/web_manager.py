...
def default_callback():
    ...
    def book_info_callback(book: Book, chapters: int):
        """指示书籍基本信息的回调函数, 同时创建一个进度条"""
        nonlocal bar
        with TLOCK:
            # 创建进度条, 并设置总长度和书籍名称
            bar = tqdm(total=chapters)
    ...
    def error_callback(exception: Exception):
        ...
        if bar is not None:
            with TLOCK:
                error_message_str = \
                    ''.join([f'\t{i}' for i in traceback.format_exception(exception)[:-1]])
                tqdm.write(
                    f"出现了一个错误:\n" \
                    f"{error_message_str}"
                )
...
class WebManager(object):
    ...
    def append_default_engine(self) -> bool:
        """加载该模块内预置的引擎"""
        # 为了防止出现循环导入错误，这里将其放在函数内进行导入
        from .web_engines import ENGINE_LIST
        # 逐个导入预置的引擎
        results = [self.append_engine(i) for i in ENGINE_LIST]
        # 检查是否出现已导入的引擎
        if False in results:
            return False
        return True
    ...
    def download(self, url: str, multi_thread: bool = False) -> Book:
        # 获取对应的引擎
...
class DownloadManager(object):
    def __init__(
        ...
        book_info_callback: Callable[[Book, int], None],
        ...
        :type engine: BookWeb
        :param book_info_callback:书籍信息回调函数, 要求输入为 Book 对象和章节数
    ...
    def download(self, only_info: bool = False, multi_thread: bool = True) -> Book:
        ...
        :type only_info: bool
        :type multi_thread: 是否要使用多线程下载, 为 True 则启用多线程
        """
        # 获取基本书籍信息
        book_info = self.__download_book_info()
        # 若只下载书籍信息则直接返回结果
        if only_info:
            return book_info[0]
        # 下载章节内容
        book = self.__download_chapters_content(*book_info, multi_thread)
        # 返回整本书籍
    ...
    def __download_book_info(self) -> Tuple[Book, List[str]]:
        ...
        # 触发书籍信息获取回调函数并返回值
        self.__book_info_callback(book, len(chapter_url_list))
    ...
    def __download_chapters_content(
        ...
        if multi_thread and self.__engine.multi_thread:
            ...
            with ThreadPoolExecutor(thread_name_prefix=f"书籍《{book.name}》下载线程") as executor:
                future_to_url = {
                    executor.submit(
                        self.__download_chapter_content, chapter_url, index, book
                    ): chapter_url
    ...
    def __download_chapter_content(self, chapter_url: str, index: int, book: Book) -> bool:
        for one_chapter in book.chapter_list:
            if (index + 1) == one_chapter.index:
                self.__chapter_info_callback(one_chapter)
                return True
        # 获取章节内容
